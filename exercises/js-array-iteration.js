...
*/
const mayHoliday = holidays.find((item) => {
  if (item.month === 'may') {
    return item
  }
})
...
*/
const doesEveryWordContainA = words.every((word) => word.search('a'))
...
*/
const doesAnyWordContainX = words.some((word) => word.search('x'))
...
*/
const developersObject = developersArray.reduce((item) => {
